...
C *********************************************************************
     ...
     >        iset, ihadron, icharge, old2, old3, ih, isu2, icp, IFINI_DEFLORIAN
      ...
      real*8 GRAD(-5:5,17) ! AP fot HKNS
      INTEGER ISET_HKNS, ICHARGE_HKNS ! AP fot HKNS
...
c     Alexei Prokudin 05/12/2009 HKNSFF
      elseif ( ffset .eq. 6) then 
C  SUBROUTINE HKNSFF(Q2,X,ISET,ICHARGE,FF,GRAD):
C
C   Subroutine HKNSFF returns the values of fragmentaion functions
C   and their gradient terms at specified Q^2 and x point 
...
* note: 3 = +&- = charge sum (NOT average) in this library             *
      ...
      if(ihadron.eq.1)then      ! PIONS
      if(fforder.eq.0)then !LO
      ISET_HKNS = 1
      norm = 1.d0
      endif
      if(fforder.eq.1)then !NLO
      ISET_HKNS = 2
      norm = 1.d0
      endif
      endif
      
      if(ihadron.eq.2)then      ! KAONS
      if(fforder.eq.0)then !LO
      ISET_HKNS = 3
      norm = 1.d0
      endif
      ...
      if(ihadron.eq.3)then      ! HADRONS --> I ASSUME THEM TO BE PIONS !!!!!!!!!!!!
      if(fforder.eq.0)then !LO
      ISET_HKNS = 1
      norm = 1.d0
      endif
      ...
      if(icharge.eq.0)then ! pi0 = (pi+ + pi-) /2
      ICHARGE_HKNS = 3
      norm = 1.d0
      else if(icharge.eq.1)then ! pi+
      ICHARGE_HKNS = 1
      norm = 1.d0
      else if(icharge.eq.2)then ! pi-
      ICHARGE_HKNS = 2
      norm = 1.d0
      else if(icharge.eq.3)then ! pi- + pi-
      ICHARGE_HKNS = 3
      norm = 2.d0
      endif
         

      call HKNSFF(Q2,Z,ISET_HKNS,ICHARGE_HKNS,dff,GRAD)
      ...
      write(6,*) 'choose ffset=1,2,3,4,5,6 for K,KKP,BFGW, FDSS, AKK,
     +HKNS'//readme
      stop      

...
C
      XT(1) = DLOG(X)
      XT(2) = DLOG(Q2)
      DUV   = DFINT_DSSV(NARG,XT,NA,ARRF,XUF)  * (1.D0-X)**3 * X
      DDV   = DFINT_DSSV(NARG,XT,NA,ARRF,XDF)  * (1.D0-X)**4 * X
      DUBAR = DFINT_DSSV(NARG,XT,NA,ARRF,XUBF) * (1.D0-X)**8 * X**0.5
      DDBAR = DFINT_DSSV(NARG,XT,NA,ARRF,XDBF) * (1.D0-X)**8 * X**0.5
      DSTR  = DFINT_DSSV(NARG,XT,NA,ARRF,XSF)  * (1.D0-X)**8 * X**0.5
      DGLU  = DFINT_DSSV(NARG,XT,NA,ARRF,XGF)  * (1.D0-X)**5 * X**0.5
C...
 60   RETURN
      END
C
...
C...
     ...
     1          XSF(NX,NQ), XGF(NX,NQ), XT(NARG), ARRF(NX+NQ) 
      ...
      character *1 dummy0
C...
      DATA fname1 / 'dssv-plus-','dssv-minus-'/
C...
      COMMON/ DSSVGRID / XUF, XDF, XUBF, XDBF, XSF, XGF, NA, ARRF
C
...
*
      FUNCTION DFINT_DSSV(NARG,ARG,NENT,ENT,TABLE)
 ...
 15   IFADR=IFADR*NENT(I)
      DFINT_DSSV=DFINT_DSSV+FAC*TABLE(IADR)
      IL=NARG
